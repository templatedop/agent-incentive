...
func generatejson(v3 *openapi3.T) {
	...
	// Traverse the entire document to replace $refs with actual schemas, with cycle guard
	inlineCount := make(map[string]int)
	traverseAndReplaceRefs(jsonParsed, jsonParsed, inlineCount, 1)

	// replaceDataType needs its own cycle detection to avoid infinite recursion
	// on already-inlined recursive structures
	visited := make(map[*gabs.Container]bool)
	replaceDataTypeWithGuard(jsonParsed, "NullString", "string", visited, 0)
...
func replaceDataType(container *gabs.Container, targetType, newType string) {
	visited := make(map[*gabs.Container]bool)
	replaceDataTypeWithGuard(container, targetType, newType, visited, 0)
}

func replaceDataTypeWithGuard(container *gabs.Container, targetType, newType string, visited map[*gabs.Container]bool, depth int) {
	// Guard against infinite recursion with depth limit
	if depth > 100 {
		return
	}
	...
	for key, child := range children {
		...
		if key == "properties" && child.Exists(targetType) {
			...
			// Safely check if the type matches targetType
			if typeVal, ok := child.Data().(string); ok && typeVal == targetType {
				// If the type is exactly the targetType (e.g., "NullString"), replace it
				container.Set(newType, "type")
			}
		} else if child != nil {
			// Recurse into nested structures with cycle guard and depth tracking
			replaceDataTypeWithGuard(child, targetType, newType, visited, depth+1)
		}
	}
}

func traverseAndReplaceRefs(container, root *gabs.Container, inlineCount map[string]int, maxInline int) {
	children, _ := container.ChildrenMap()

	for key, child := range children {
		// Check for $ref and replace it with the actual schema
		if key == "$ref" {
			// Safely read the ref path
			refVal := child.Data()
			...
			// Guard against infinite expansion of self-referential schemas
			if inlineCount[refPath] >= maxInline {
				// Leave the $ref as-is to avoid deep recursion
				continue
			}

			if schema := resolveSchema(refPath, root); schema != nil {
				// Replace the $ref with the actual schema (one level only)
				inlineCount[refPath]++
			...
			traverseAndReplaceRefs(child, root, inlineCount, maxInline)
		}
	}
}
