...
func buildDocs(eds []EndpointDef, cfg *config.Config) *openapi3.T {
	// Determine if we are in build mode via API or config
	isBuildMode := GetGenerationMode() == BuildMode || strings.EqualFold(cfg.GetString("swagger.generation.mode"), "build")

	if isBuildMode {
		// Build mode: generate the doc and write files
		...
		// Persist generated v3 document files asynchronously (non-blocking)
		go func() {
			// Save canonical pre-generated swagger for future build-mode startups
			// if err := SavePreGeneratedSwagger(v3Doc); err != nil {
			// 	fmt.Println("Error saving pre-generated swagger:", err)
			// }
			// Also store v3Doc.json for inspection/debugging if desired
	...
	// Runtime mode: do NOT generate and do NOT load pre-generated file
	fmt.Println("Runtime mode: skipping swagger generation and file loading")
	return nil
...
var builtinNullableTypeMap = map[reflect.Type]m{
	...
	reflect.TypeOf(null.Int32{}):   {"type": "integer", "format": "int32"},
	reflect.TypeOf(null.Uint{}):    {"type": "integer", "format": "uint32"},
	reflect.TypeOf(null.Uint32{}):  {"type": "integer", "format": "uint32"},
