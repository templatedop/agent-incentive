...
import (
	...
	"gitlab.cept.gov.in/it-2.0-common/n-api-server/util/diutil/typlect"
...
func isEmptyStruct[T any](x T) bool {
...
func buildImproved[Req, Res any](f HandlerFunc[Req, Res], defaultStatus ...int) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		if hasInput {
			...
			// Special rule for GET: if a Validator is present, run it; otherwise skip validation.
			// For all non-GET methods, a Validator is required and must pass.
			if v, ok := any(req).(Validator); ok {
				if err := v.Validate(); err != nil {
					apierrors.HandleGoValidError(c, err)
					return
				}
			} else if vptr, ok := any(&req).(Validator); ok {
				// Support pointer receiver Validate() implementations
				if err := vptr.Validate(); err != nil {
					apierrors.HandleGoValidError(c, err)
					return
				}
			} else if c.Request.Method == http.MethodGet {
				// GET without a Validator: skip validation
			} else {
				// Non-GET without a Validator: return error
				apierrors.ErrorResponseWithStatusCodeAndMessage(
					c,
					apierrors.NewHTTPStatusCodeAndMessage(http.StatusUnprocessableEntity, "request must implement Validator.Validate()"),
					"Validator not implemented on request DTO",
					nil,
				)
				return
			}
...
func bindRequestBody[Req any](c *gin.Context, ctx *Context, req *Req) error {
	...
	if rawCT == "" {
		...
		codeAndMsg := apierrors.NewHTTPStatusCodeAndMessage(http.StatusBadRequest, msg)
...
func routeToContentTypeBinder[Req any](c *gin.Context, ctx *Context, req *Req, mediaType string) error {
	...
	default:
		...
		codeAndMsg := apierrors.NewHTTPStatusCodeAndMessage(http.StatusUnsupportedMediaType, msg)
